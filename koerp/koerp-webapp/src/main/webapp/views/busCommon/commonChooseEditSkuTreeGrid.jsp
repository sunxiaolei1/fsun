...
<div id="skuToolbar" style="dispaly:none;" >	
	...
	<div style="height:24px;">
		<span class="skutitle" >
			商品(或物料)列表[<span id="skutitle"></span>]
		</span>
	</div>		
</div>
...
$(function() {
        ...
        onExpandRow: function(index,row){ 
			...
			//if(!hasExpand(row)){
				$('#detailForm-'+index).panel({  
	                ...
	                border:false,  
	                cache:false,  
	               	href:'${api}/bus/basSku/toMaterielsView?currIndex='+ index +'&skuId='+ row.skuId +"&tradeType="+ currTradeType,
	                onLoad:function(){
	...
	initCategoryTree({"enabled": 1}, 
		function(node){			   
			...
			currDatagrid.datagrid('options').url = "${api}/bus/basSku/refund/findListForPage";  
...
function delSkuOne(rowIndex){
	...
	for(var i in currCheckedSkus) {	
		if(existSku(currCheckedSkus[i], rowData.sku)) {
			...
			currDatagrid.datagrid("collapseRow", rowIndex);
			currDatagrid.datagrid("refreshRow", rowIndex);						
			//刷新父页面商品列表
			skuListReLoad();
			break;
