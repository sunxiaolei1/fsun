...
<div id="stockToolbar" class="datagrid-toolbar" style="display: none;padding: 5px;"> 
    <input type="text" id="stockQ" style="width: 150px;" class="easyui-textbox" data-options="prompt:'输入仓库代码或名称检索...'"/>
    <a href="#" class="easyui-linkbutton" iconCls="icon-2012092109942" onclick="stockSearch()">查询</a>
    <a href="#" class="easyui-linkbutton" iconCls="icon-20130406015709810_easyicon_net_16" onclick="stockClear()">重置</a>
    <div style="clear: both"></div>
</div>
...
var companyInitFirst = true;
var shopInitFirst = true;
var stockInitFirst = true;
 
function initCompanyGrid(isEdit, defaultCompanycode, defaultShopid, defaultStockcode){
	...
	if(isEdit){
		$('#companycode').combogrid({
	        ...
	        method: 'post',
	        queryParams: {"isenable": true, "q": defaultCompanycode},
	        multiple: false,
	        fitColumns: true,
	        striped: true,
	        ...
	        remoteSort:true,
	        editable:false,  
	        url: '${api}/company/findCompanyPageList',
	        ...
	        onLoadSuccess:function(data){ 	        	
	        	//$('#companycode').combogrid('grid').datagrid('selectRecord',defaultCompanycode);
	        	if(companyInitFirst){
	        		 $('#companycode').combogrid('setValue',defaultCompanycode);
	                 initShopGrid(isEdit, defaultShopid);
	                 initStockGrid(isEdit, defaultStockcode);
	        		 companyInitFirst = false;
	        	}
	        ...
	        onSelect:function(rowIndex, rowData){
	        	...
	        	var stockParams = $("#stockcode").combogrid("options").queryParams;
	        	shopParams.companycode = companycode;
	        	stockParams.companycode = companycode;
	        	shopClear();
	        	stockClear();
	        },
	        onChange:function(newValue, oldValue) {
				if(newValue!=null && newValue!=''){
					$("#shopid").combogrid("enable");
					$("#stockcode").combogrid("enable");
				}else{
					$("#shopid").combogrid("disable");
					$("#stockcode").combogrid("disable");
				}									
			}
	    });
	}else{
...
function allForCompanyInit() {
	...
	stockParams.companycode = '';
	shopClear();
	stockClear();
}


function initShopGrid(isEdit, defaultShopid) {
	if(isEdit){		
		$('#shopid').combogrid({
			prompt:'请选择...',
	    	panelWidth:500,
	        ...
	        onLoadSuccess:function(data){  
	        	if(shopInitFirst){
	        		$("#shopid").combogrid('setValue',defaultShopid);
	        		shopInitFirst = false;
	        	}               
            },
	        onSelect:function(rowIndex, rowData){
...
function shopClear() {
	$("#shopQ").textbox("setValue","");
	$("#shopid").combogrid('setValue',"");
	var shopParams = $("#shopid").combogrid("options").queryParams;
	shopParams.q = '';
	$('#shopid').combogrid('grid').datagrid("reload");	
}

function initStockGrid(isEdit, defaultStockCode) {
	...
	if(isEdit){
		$('#stockcode').combogrid({
			...
			panelWidth:500,
			panelHeight:300,
	        idField: 'stockcode', //ID字段
	        textField: 'stockname', //显示的字段
	        method: 'post',       
	        multiple: false,
		    fitColumns: true,  
		    striped: true,  
	        ...
	        url: '${api}/stock/findStockPageList',
	        pageSize: GLOBAL_PAGE_SIZE,
	        pageList: GLOBAL_PAGE_SIZE_LIST,
	        toolbar:'#stockToolbar',  
	        columns: [[
	            {field: 'stockid', checkbox: true},
	            {field: "companycode", title: "公司代码", width: 30, align: "center", sortable:true},
	            {field: "stockcode", title: "仓库代码", width: 80, align: "center", sortable:true},
	            {field: "stockname", title: "仓库名称", width: 100, align: "center", sortable:true}
	        ]],
	        loadMsg: "数据加载中请稍后……",
	        emptyMsg: "没有符合条件的记录",	
	        onLoadSuccess:function(data){  
	        	if(stockInitFirst){
	        		$('#stockcode').combogrid('setValue',defaultStockCode);
	        		stockInitFirst = false;
	        	}                 
            },
	        onSelect:function(rowIndex, rowData){
	        	$("#stockQ").textbox("setValue","");
	        }
	    });
	}else{
		$('#stockcode').combogrid({
	        ...
	        emptyMsg: "没有符合条件的记录",
	        onShowPanel: function () {
	        	$('#stockcode').combogrid('grid').datagrid('options').url = '${api}/stock/findStockPageList';
	        	$('#stockcode').combogrid('grid').datagrid('reload');
	        },
	        onSelect:function(rowIndex, rowData){
	        	$("#stockQ").textbox("setValue","");
	        }
