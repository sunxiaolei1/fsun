					        ...
					        <tr> 
								...
								<td>
									<input id="address" name="address" data-options="multiline:true" 
										class="easyui-textbox" style="height:46px;"/>
								</td>       	
					        	<th>注意事项</th>
								<td colspan="3">
...
$(function () { 
	...
	$.ajax({
		type : "GET",
		url : "${api}/bus/order/getInitCopyOrder",
		data:{
			"orderId":"${orderId}",
			"orderType": "${orderType}"
		},
		...
		success : function(result) {		
			...
			var carrierId = header.carrierId;
			var buyerId = header.buyerId;
			//debugger
			if(header!=null){
				delete header.carrierId;
				delete header.buyerId;
				$orderfm.form("load", header);	
				$("#orderId0", $orderfm).text(header.orderId);
			}
			
			initSalesmanGrid(carrierId, $orderfm);
			...
			initAllCustomerGrid(buyerId, $orderfm);
