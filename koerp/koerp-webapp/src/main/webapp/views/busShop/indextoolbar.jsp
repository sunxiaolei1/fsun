...
//跳转至新增界面
function toAddView(){		
	commonDialog("ordersDialog", "创建店仓", "75%", "240px", 
...
function toDetailView(){
	...
	commonDialog("ordersDialog", "店仓("+ rows[0].shop_name +")明细", "75%", "240px", 
...
function changeStatus(rows, status, title){
	...
	$.each(rows,function(){
		shopIds.push(this.shop_id);
	});
	$.messager.confirm("确认","是否确认"+ title +"操作?",function(sure){
		if(sure) {		
			commonPost("${api}/bus/shop/status/"+ status +"?shopIds="+shopIds.join(","), 
				{}, reflushDataGrid);		
		}
	});
}
